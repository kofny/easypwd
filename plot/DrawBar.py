...
def plot_bar(items: List[CellItem]):
    ...
    # print(ax[0])
    ends = [140000, 750000, 140000, 1000000]
    lims = [180000, 950000, 180000, 1065000]
    end_labels = [r'$1.5\times10^{5}$', r'$7.5\times10^{5}$', r'$1.5\times10^{5}$', r'$10^{6}$']
    legend = ["Leet", "Syllable", "Keyboard", "Date"]
    for i, each_subplot in enumerate(four_subplots):
        # axes[i].set_xscale('log')
        # print(axes[i].get_xticks())
        axes[i].tick_params(axis='x', which='major', direction="in")
        ...
        for j in range(len(each_subplot), 0, -1):
            ...
            axes[i].barh([item.index], [item.data], item.bar_width, left=[item.offset],
                         color=item.color if item.hatch is None else "black",
                         hatch=None)
        pass

        print(axes[i].get_xlim())
        axes[i].set_xlim([0, lims[i]])
        axes[i].set_xlabel(f'Number of {legend[i]}')
...
def main():
    ...
    index2 = np.arange((2 * config.types - 1) * config.bar_width, 0, -2 * config.bar_width)
...
def draw_legend():
    ...
    plt.barh([1], [0], color='red',
             label="Cracked by other methods")
    plt.barh([2], [0], color='black', label="Extra cracked by CKL methods")
    legend = plt.legend(loc='center', fontsize=24, handlelength=3, handletextpad=1.6, frameon=False, labelspacing=0.7,
                        ncol=2)
